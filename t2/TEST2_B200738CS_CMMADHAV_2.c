...
void update(Node* tree, const int k){
	Node* f = searchBST(tree, k);
	f->vacstat = !f->vacstat;
}
int traverse(Node* n, int i){
	if( n != NULL){
		int left = traverse(n->left, i);
		int mid;
		if(n->vacstat == i)
			mid =1	;
		else
			mid =0;
		int right = traverse(n->right,i);
...
void findDiff(Node*t, const int k){
	Node* n = searchBST(t,k);
	if(n == NULL){
		return;
	}
	n = n->right;
	...
	int diff = abs(traverse(n,1) - traverse(n,0));
	printf("%d\n", diff);
}
void printTree(Node *root, const int k)
{
    if (root == NULL)
    {
        printf("( ) ");
		...
		printf("( %d %d %d ", root->data, traverse(root->right, 0) ,traverse(root->right, 1));
